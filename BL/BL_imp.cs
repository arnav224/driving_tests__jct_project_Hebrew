...
namespace BL
{
    public class BL_imp : IBL
    {
        ...
        {
            IDAL.UpdateEmailSendingTime(testID);
        }
        ...
        public void SendEmail(int TestID, EmailType emailType, string NoteToAdd = "")
        {
            if (BE.Configuration.AutoSendingEmailsAboutAddingAndCancalation)
            {
                Test test = GetAllTests(t => t.TestID == TestID).First();
                new Thread(() =>
                {
                    try
                    {
                        Trainee trainee = GetAllTrainees(t => t.ID == test.TraineeID).First();
                        string subject = "";
                        string body = IDAL.GetEmailTemltateHTML(emailType);
                        switch (emailType)
                        {
                            case EmailType.Reminder:
                                subject = "מועד הטסט שלך מתקרב";
                                if (BE.Tools.SendingEmail(trainee.MailAddress, subject, IDAL.GetEmailTemltateTestRemeinder(TestID, NoteToAdd)))
                                    IDAL.UpdateEmailSendingTime(TestID);
                                return;
                            case EmailType.TestAdded:
                                subject = "נרשמת בהצלחה לטסט";
                                body = body
                            ...
                            case EmailType.Appeal:
                                if (test.AppealTest.appealStatus == AppealStatus.ממתין)
                                {
                                    subject = "הגשת ערעור על הטסט שלך";
                                    body = body
                                    .Replace("@@Name@@", trainee.FirstName + (trainee.Gender == BE.Gender.זכר ? " היקר " : " היקרה "))
                                    .Replace("@@DATE@@", test.Time.ToString("dd/MM/yyyy"))
                                    .Replace("@@TITLE@@", subject)
                                    .Replace("@@NOTES@@", "מצב הערעור הוא כרגע \"ממתין\".")
                                    .Replace("@@TESTID@@", test.TestID.ToString());
                                }
                                else
                                {
                                    subject = "התקבלה תשובה על הערעור שלך";
                                    body = body
                                    .Replace("@@Name@@", trainee.FirstName + (trainee.Gender == BE.Gender.זכר ? " היקר " : " היקרה "))
                                    .Replace("@@DATE@@", test.Time.ToString("dd/MM/yyyy"))
                                    .Replace("@@TITLE@@", subject)
                                    .Replace("@@NOTES@@", "סטטוס הערעור הוא כרגע " + '"' + test.AppealTest.appealStatus + '"')
                                    .Replace("@@TESTID@@", test.TestID.ToString());
                                }
                                break;
                            default:
                        ...
                        BE.Tools.SendingEmail(trainee.MailAddress, subject, body);
